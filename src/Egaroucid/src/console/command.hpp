...
void check_command(Board_info *board, State *state, Options *options, int actual = 0) {
    ...
    std::string cmd, arg, arg_mode;
    printf("actual : %d\n", actual);
    //------------------------------------------------
    int compt = 0;
    char file_content[11] = {0};
    ...
    while (!(actual == compt)) 
    {
        ...
        if (fgets(file_content, sizeof(file_content), file) == nullptr) {
            fclose(file);
            throw std::runtime_error("Erreur de lecture du fichier");
        }
        fclose(file);
        compt = (file_content[2] - '0') * 10 + (file_content[3] - '0');
    }

    int cmd_id;

    if (file_content[1] == '.') {
        // Convertir le 1er caractère en entier
        cmd_id = file_content[0] - '0';
    } 
    else {
        // Vérifier que les 2 premiers caractères sont des chiffres
        // Conversion manuelle
        if (isdigit(file_content[0]) && isdigit(file_content[1])) {
            cmd_id = (file_content[0] - '0') * 10 + (file_content[1] - '0');
        }
        else {
            throw std::runtime_error("Caractères invalides");
        }
    }
    arg = std::string(file_content + 4, 2);
    size_t len = strlen(file_content);
    arg_mode = std::string(1, file_content[len - 1]);
    printf("Commande : %d\n", cmd_id);
    //------------------------------------------------
